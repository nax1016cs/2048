...
public:	
	...
	void train_weight(const std::array<std::pair<board , board::reward> ,3 > ,int last){
		double rate = 0.1/(tuple_num * 8);
		double gt = record[0].second + record[1].second*rate +record[2].second*rate*rate +  (board_value(record[2].first)-board_value(record[1].first))*rate*rate*rate;
		double v_s = last ? 0 :  rate * (gt-(board_value(record[2].first)-board_value(record[1].first)))	;
		for(int i=0; i<tuple_num; i++){
			for(int l=0; l<4; l++){
				rotate_right();
				for(int m=0; m<4; m++){
					net[i][caculate_tuple_value(record[0].first,i)]+= v_s;	
					reflection();
				}
			}
		}
